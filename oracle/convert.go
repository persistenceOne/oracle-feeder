	...
	for pairProviderName, pairs := range providerPairs {
		for _, pair := range pairs {
			if strings.ToUpper(pair.Quote) != config.DenomUSD {
				...
				validCandleList, err := getValidCandles(candles, validProviders, pair.Quote)
				if err != nil {
					return nil, err
				}
...
func getValidCandles(candles provider.AggregatedProviderCandles, validProviders map[provider.Name]struct{}, quote string) (provider.AggregatedProviderCandles, error) {
	...
	if len(validCandleList) == 0 {
		return nil, fmt.Errorf("there are no valid conversion rates for %s", quote)
	}

	return validCandleList, nil
}
