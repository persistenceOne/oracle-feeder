...
func NewKrakenProvider(
	...
	krakenLogger := logger.With().Str("provider", string(Kraken)).Logger()
...
func (p *KrakenProvider) SubscribeCurrencyPairs(cps ...types.CurrencyPair) error {
	...
	var newPairs []types.CurrencyPair
	for _, cp := range cps {
...
func (p *KrakenProvider) getCandlePrices(key string) ([]types.CandlePrice, error) {
	...
	var candleList []types.CandlePrice
...
func (p *KrakenProvider) messageReceivedTickerPrice(bz []byte) error {
	...
	telemetryWebsocketMessage(Kraken, MessageTypeTicker)
	return nil
}
...
func (p *KrakenProvider) messageReceivedCandle(bz []byte) error {
	...
	telemetryWebsocketMessage(Kraken, MessageTypeCandle)
...
func (ticker KrakenTicker) toTickerPrice(symbol string) (types.TickerPrice, error) {
	...
	return types.NewTickerPrice(string(Kraken), symbol, ticker.C[0], ticker.V[1])
}
...
// ex.: ATOM/USD and return currencyPair Symbol ATOMUSD.
...
// and return kraken ticker symbol ATOM/USD.
