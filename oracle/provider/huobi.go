...
func NewHuobiProvider(
	...
	huobiLogger := logger.With().Str("provider", string(Huobi)).Logger()
...
func (p *HuobiProvider) getSubscriptionMsgs(cps ...types.CurrencyPair) []interface{} {
	subscriptionMsgs := make([]interface{}, 0, len(cps)*2) //nolint: gomnd //const
	for _, cp := range cps {
...
func (p *HuobiProvider) messageReceived(messageType int, bz []byte) {
	...
	if tickerResp.Tick.LastPrice != 0 {
		...
		p.logger.Debug().
			Str("provider", Huobi.String()).
			Str("messageType", messageTypeTicker).
	...
	if candleResp.Tick.Close != 0 {
		...
		p.logger.Debug().
			Str("provider", Huobi.String()).
			Str("messageType", messageTypeCandle).
			Msg("Message received")
...
func (ticker HuobiTicker) toTickerPrice() (types.TickerPrice, error) {
	return types.NewTickerPrice(
		string(Huobi),
		ticker.CH,
		strconv.FormatFloat(ticker.Tick.LastPrice, 'f', -1, 64), //nolint: gomnd //const
		strconv.FormatFloat(ticker.Tick.Vol, 'f', -1, 64),       //nolint: gomnd //const
	)
}
...
func (candle HuobiCandle) toCandlePrice() (types.CandlePrice, error) {
	return types.NewCandlePrice(
		string(Huobi),
		candle.CH,
		strconv.FormatFloat(candle.Tick.Close, 'f', -1, 64),  //nolint: gomnd //const
		strconv.FormatFloat(candle.Tick.Volume, 'f', -1, 64), //nolint: gomnd //const
